...
def create_cli_app(
    coordinator: AgentCoordinator, config: SystemConfig
    ...
    async def run_cli():
        ...
        try:
            while True:
                    ...
                    if result.get("needs_clarification"):
                        ...
                        answer = Prompt.ask(
                            ...
                            + (
                                "选项:\n"
                                + "\n".join(
                                    f"- {opt}" for opt in clarification["options"]
                                )
                        ...
                        if result.get("documentation"):
                            ...
                            if Confirm.ask("\n是否保存文档到文件？"):
                                output_path = Prompt.ask(
                                    "请输入保存路径",
                                    _default="output/requirements_doc.json",
                                )
                                output_file = Path(output_path)
                                output_file.parent.mkdir(_parents=True, exist_ok=True)
                ...
                _text = None  # Reset text for next iteration
