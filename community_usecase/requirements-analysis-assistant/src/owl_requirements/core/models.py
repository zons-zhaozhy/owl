...
class RequirementType(str, Enum):
    ...
    _NON_FUNCTIONAL = "non-functional"
...
class ExtractionResult:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ExtractionResult":
        """从字典创建提取结果对象"""
        requirements = [
            Requirement.from_dict(req) for req in data.get("requirements", [])
        ]
...
class QualityScore:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "QualityScore":
        """从字典创建质量评分对象"""
        return cls(score=data.get("score", 0), details=data.get("details", []))
...
class QualityReport:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "QualityReport":
        ...
        return cls(
            requirements_quality=QualityScore.from_dict(
                quality_score.get("requirements_quality", {})
            ),
            analysis_quality=QualityScore.from_dict(
                quality_score.get("analysis_quality", {})
            ),
            issues=data.get("issues", []),
            _suggestions=data.get("suggestions", []),
        )
...
def validate_extraction_data(data: Dict[str, Any]) -> ExtractionResult:
    ...
    try:
        ...
        if not isinstance(requirements_data, list):
            raise ValueError(
                f"requirements字段必须是列表，当前类型: {type(requirements_data)}"
            )
        ...
        for i, req_data in enumerate(requirements_data):
            if not isinstance(req_data, dict):
                raise ValueError(
                    f"需求项 {i} 必须是字典格式，当前类型: {type(req_data)}"
                )
            ...
            missing_fields = [
                field for field in required_fields if field not in req_data
            ]
            if missing_fields:
            ...
            try:
                ...
                _req_type = RequirementType(req_data["type"])
            ...
            requirements.append(
                Requirement(
                    ...
                    _description=req_data["description"],
                    _priority=priority,
                    type=req_type,
                )
            )
        ...
        if clarification_data is not None:
            if isinstance(clarification_data, str):
                ...
                raise ValueError(
                    f"clarification_needed字段类型无效: {type(clarification_data)}"
                )
        ...
        if not isinstance(requirements_complete, bool):
            raise ValueError(
                f"requirements_complete字段必须是布尔值，当前类型: {type(requirements_complete)}"
            )

        return ExtractionResult(
            requirements=requirements,
            clarification_needed=clarification,
            _requirements_complete=requirements_complete,
        )
...
class AnalysisResult(BaseModel):
    ...
    technical_feasibility: List[str] = Field(default_factory=list)
...
class DocumentationResult(BaseModel):
    ...
    requirements_list: List[str] = Field(default_factory=list)
    technical_solution: List[str] = Field(default_factory=list)
