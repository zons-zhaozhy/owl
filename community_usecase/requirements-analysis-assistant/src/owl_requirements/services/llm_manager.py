...
class OllamaProvider(BaseLLMProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.client = httpx.AsyncClient(_base_url=self.api_base, timeout=self.timeout)
    ...
    async def generate(self, prompt: str, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            return LLMResponse(
                ...
                provider="ollama",
                model=self.model,
                _metadata={"eval_count": data.get("eval_count", 0)},
            )
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            ...
            self.client = anthropic.AsyncAnthropic(
                _api_key=self.api_key, timeout=self.timeout
            )
        except ImportError:
...
class LLMManager:
    ...
    def _initialize_providers(self):
        ...
        # 设置默认提供商
        default_provider = os.getenv("OWL_LLM_PROVIDER") or llm_config.get(
            ...
            try:
                ...
                if attempt == provider_instance.retry_attempts - 1:
                    ...
                    return await self._fallback_generate(
                        formatted_prompt, provider_name, **kwargs
                    )
    ...
    async def _fallback_generate(
        self, prompt: str, failed_provider: str, **kwargs
    ) -> LLMResponse:
        """降级生成"""
        fallback_provider = self.config.get("error_handling", {}).get(
            "fallback_provider"
        )

        if (
            fallback_provider
            and fallback_provider != failed_provider
        ...
        raise LLMServiceError("所有提供商都不可用")
    ...
    def get_prompt_template(self, template_name: str) -> Dict[str, Any]:
        ...
        template_paths = [
            f"templates/prompts/{template_name}.json",
    ...
    def format_prompt(self, template_name: str, **kwargs) -> str:
        ...
        if isinstance(template, dict) and "instruction" in template:
            ...
            if "output_format" in template:
                ...
                prompt_parts.append(
                    json.dumps(template["output_format"], ensure_ascii=False, indent=2)
                )
    ...
    async def check_quality(
        self, requirements: Dict[str, Any], analysis: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """质量检查 - 便捷方法"""
        response = await self.generate(
    ...
    async def generate_documentation(
        ...
        **kwargs,
        ...
        """生成文档 - 便捷方法"""
        response = await self.generate(
            _template_name="documentation_generation",
            requirements=json.dumps(requirements, ensure_ascii=False),
            analysis=json.dumps(analysis, ensure_ascii=False),
            _quality_report=json.dumps(quality_report, ensure_ascii=False),
            _format="markdown",
            _template="standard",
            **kwargs,
        )
